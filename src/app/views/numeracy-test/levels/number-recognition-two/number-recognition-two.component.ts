...
import { loadNumberRecognitionTwoLevelResult } from '../../store/number-recognition-two-level-result/number-recognition-two-level-result.actions';
import { NumberRecognitionTwoLevelResult } from '../../store/number-recognition-two-level-result/number-recognition-two-level-result.model';
import { numberRecognitionTwoLevelResultIsLoading, selectNumberRecognitionTwoLevelResult } from '../../store/number-recognition-two-level-result/number-recognition-two-level-result.selectors';
...
export class NumberRecognitionTwoComponent implements OnInit {
  ...
  constructor(
    ...
    private store: Store<NumberRecognitionTwoLevelResult>
  ) {}

  ngOnInit(): void {
    let letterResultLoading$: Observable<any> = this.store.pipe(
      select(numberRecognitionTwoLevelResultIsLoading)
    );
    letterResultLoading$.subscribe((data: any) => {
      this.isLoadingStarCards = data;
    });
    this.userData$ = this.store.pipe(select(selectNumberRecognitionTwoLevelResult));
  ...
  onGetGameLevelResult(GameSessionId: string) {
    this.store.dispatch(loadNumberRecognitionTwoLevelResult({ session_id: GameSessionId }));
